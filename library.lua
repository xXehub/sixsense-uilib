...
local BaseURL = "https://raw.githubusercontent.com/xXehub/sixsense-uilib/refs/heads/main/"
...
local FetchIcons, Icons = pcall(function()
    return (loadstring(
        -- game:HttpGet("https://raw.githubusercontent.com/xXehub/sixsense-uilib/refs/heads/main/lucide-source.lua")
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if KeyPicker.SyncToggleState then
            -- SyncToggleState no longer restricts modes, just syncs toggle state
            -- Info.Modes = { "Toggle" }
            -- Info.Mode = "Toggle"
        end
...
function Library:CreateWindow(WindowInfo)
    ...
    local function ApplySidebarLayout()
        ...
        local ProfileHeight = LayoutRefs.UserProfileHolder and LayoutRefs.UserProfileHolder.Visible and math.floor(56 * Library.DPIScale) or 0
        ...
        if LayoutRefs.TabsFrame then
            LayoutRefs.TabsFrame.Size = UDim2.new(0, SidebarWidth, 1, -math.floor(70 * Library.DPIScale) - ProfileHeight)
        end

        if LayoutRefs.UserProfileHolder then
            LayoutRefs.UserProfileHolder.Size = UDim2.new(0, SidebarWidth, 0, ProfileHeight)
        end

        if LayoutRefs.UserInfoContainer then
            LayoutRefs.UserInfoContainer.Visible = not IsCompact
        end

        if LayoutRefs.ContainerFrame then
            LayoutRefs.ContainerFrame.Position = UDim2.fromOffset(SidebarWidth, math.floor(49 * Library.DPIScale))
            LayoutRefs.ContainerFrame.Size = UDim2.new(1, -SidebarWidth, 1, -math.floor(70 * Library.DPIScale))
        end

        if LayoutRefs.SidebarGrabber then
            LayoutRefs.SidebarGrabber.Position =
                UDim2.fromOffset(SidebarWidth - LayoutRefs.SidebarGrabber.Size.X.Offset / 2, math.floor(49 * Library.DPIScale))
        end
    ...
    do
        ...
        local UserProfileHolder = New("Frame", {
            ...
            BackgroundColor3 = "BackgroundColor",
            Position = UDim2.new(0, 0, 1, -20),
            Size = UDim2.new(0, InitialSidebarWidth, 0, 0),
            ClipsDescendants = true,
            Visible = false,
            Parent = MainFrame,
        })
        LayoutRefs.UserProfileHolder = UserProfileHolder

        local UserProfileContainer = New("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 56),
            Parent = UserProfileHolder,
        })
        Library:UpdateDPI(UserProfileContainer, { Size = UDim2.new(1, 0, 0, 56) })
        LayoutRefs.UserProfileContainer = UserProfileContainer
        
        local ProfilePadding = New("UIPadding", {
            PaddingBottom = UDim.new(0, 8),
        ...
        Library:UpdateDPI(ProfilePadding, {
            PaddingBottom = UDim.new(0, 8),
            PaddingLeft = UDim.new(0, 10),
            PaddingRight = UDim.new(0, 10),
            PaddingTop = UDim.new(0, 8),
        ...
        -- Divider line at top
        local ProfileDivider = Library:MakeLine(UserProfileHolder, {
            Position = UDim2.fromOffset(8, 0),
        ...
        Library:UpdateDPI(AvatarHolder, { Size = UDim2.fromOffset(40, 40) })
        LayoutRefs.AvatarHolder = AvatarHolder
        New("UICorner", {
            CornerRadius = UDim.new(1, 0),
            Parent = AvatarHolder,
        })
        ...
        Library:UpdateDPI(AvatarStroke, { Thickness = 1 })
        ...
        LayoutRefs.UserAvatar = AvatarImage

        -- User info container (name + status)
        ...
        Library:UpdateDPI(StatusBadge, { Position = UDim2.fromOffset(0, 20), Size = UDim2.fromOffset(45, 16) })
        local StatusCorner = New("UICorner", {
            CornerRadius = UDim.new(0, 4),
            Parent = StatusBadge,
        })
        Library:UpdateDPI(StatusCorner, { CornerRadius = UDim.new(0, 4) })
        ...
        local StatusLabel = New("TextLabel", {
            ...
            Text = "FREE",
            TextSize = 10,
            TextColor3 = Library.Scheme.MainColor,
            Parent = StatusBadge,
        })
        ...
        Library:UpdateDPI(LogoutButton, { Size = UDim2.fromOffset(32, 32) })
        local LogoutCorner = New("UICorner", {
            CornerRadius = UDim.new(0, 6),
            Parent = LogoutButton,
        })
        Library:UpdateDPI(LogoutCorner, { CornerRadius = UDim.new(0, 6) })
        
        if LogoutIcon then
            local LogoutIconImage = New("ImageLabel", {
                AnchorPoint = Vector2.new(0.5, 0.5),
                BackgroundTransparency = 1,
                Image = LogoutIcon.Url,
                ImageColor3 = "FontColor",
                ImageRectOffset = LogoutIcon.ImageRectOffset,
                ImageRectSize = LogoutIcon.ImageRectSize,
                Position = UDim2.fromScale(0.5, 0.5),
                Size = UDim2.fromOffset(16, 16),
                Parent = LogoutButton,
            })
            Library:UpdateDPI(LogoutIconImage, { Size = UDim2.fromOffset(16, 16) })
            LayoutRefs.LogoutIcon = LogoutIconImage
        end
        LayoutRefs.LogoutButton = LogoutButton

        -- Hover effects for logout button
        LogoutButton.MouseEnter:Connect(function()
            TweenService:Create(LogoutButton, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
                BackgroundColor3 = Library.Scheme.AccentColor
            }):Play()
            if LayoutRefs.LogoutIcon then
                TweenService:Create(LayoutRefs.LogoutIcon, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
        ...
        LayoutRefs.LogoutCallback = function(cb)
            LogoutCallback = cb
        end
    ...
    function Window:SetUserProfile(Info)
        --[[
            Info = {
                ...
                UserId = number (for avatar),
                Status = string ("Premium", "Free", etc),
                StatusColor = Color3 (optional, defaults to AccentColor),
                OnLogout = function (callback when logout is clicked)
            }
        ]]
        Info = Info or {}
        
        if not LayoutRefs.UserProfileHolder then return end

        -- Show the profile section
        LayoutRefs.UserProfileHolder.Visible = true

        -- Set avatar
        if Info.UserId and LayoutRefs.UserAvatar then
            local avatarUrl = string.format(
                "https://www.roblox.com/headshot-thumbnail/image?userId=%d&width=150&height=150&format=png",
                Info.UserId
            )
            LayoutRefs.UserAvatar.Image = avatarUrl
        elseif Info.Avatar and LayoutRefs.UserAvatar then
        ...
        -- Set status
        if Info.Status and LayoutRefs.StatusLabel then
            LayoutRefs.StatusLabel.Text = string.upper(Info.Status)
            
            -- Adjust badge width based on text (with DPI scaling)
            local scaledTextSize = math.floor(10 * Library.DPIScale)
            local textWidth = Library:GetTextBounds(string.upper(Info.Status), Library.Scheme.Font, scaledTextSize, 100)
            local badgeWidth = math.floor(math.max(textWidth + 12, 40) * Library.DPIScale)
            local badgeHeight = math.floor(16 * Library.DPIScale)
            LayoutRefs.StatusBadge.Size = UDim2.fromOffset(badgeWidth, badgeHeight)
        end

        -- Set status color
        if Info.StatusColor and LayoutRefs.StatusBadge then
            LayoutRefs.StatusBadge.BackgroundColor3 = Info.StatusColor
        elseif LayoutRefs.StatusBadge then
            -- Default colors based on status
            local status = (Info.Status or ""):lower()
            if status == "premium" or status == "vip" then
                LayoutRefs.StatusBadge.BackgroundColor3 = Color3.fromRGB(255, 185, 0) -- Gold
            elseif status == "admin" or status == "owner" then
                LayoutRefs.StatusBadge.BackgroundColor3 = Color3.fromRGB(255, 85, 85) -- Red
            elseif status == "free" then
                LayoutRefs.StatusBadge.BackgroundColor3 = Color3.fromRGB(100, 100, 100) -- Gray
            else
                LayoutRefs.StatusBadge.BackgroundColor3 = Library.Scheme.AccentColor
            end
        end

        -- Set logout callback
        if Info.OnLogout and LayoutRefs.LogoutCallback then
            LayoutRefs.LogoutCallback(Info.OnLogout)
        end

        -- Update layout to accommodate profile
        ApplySidebarLayout()
    end

    function Window:HideUserProfile()
        if LayoutRefs.UserProfileHolder then
            LayoutRefs.UserProfileHolder.Visible = false
            ApplySidebarLayout()
        end
    end
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddAccordion(Info)
            -- Same structure as Groupbox but with accordion functionality
            local BoxHolder = New("Frame", {
                AutomaticSize = Enum.AutomaticSize.Y,
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 0),
            ...
            local AccordionIconPlus
            local AccordionIconMinus
            local AccordionLine

            -- Get Lucide icons for plus and minus
            ...
            do
                ...
                AccordionLine = Library:MakeLine(GroupboxHolder, {
                    Position = UDim2.fromOffset(0, 34),
                    Size = UDim2.new(1, 0, 0, 1),
                })
                AccordionLine.BackgroundTransparency = 1

                -- Clickable header for accordion
                AccordionButton = New("TextButton", {
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, 34),
                    Text = "",
                    Parent = GroupboxHolder,
                ...
                -- Accordion Plus icon (shown when closed)
                if PlusIcon then
                    AccordionIconPlus = New("ImageLabel", {
                        Image = PlusIcon.Url,
                        ImageColor3 = "AccentColor",
                        ImageRectOffset = PlusIcon.ImageRectOffset,
                        ImageRectSize = PlusIcon.ImageRectSize,
                        BackgroundTransparency = 1,
                        AnchorPoint = Vector2.new(0.5, 0.5),
                ...
                -- Accordion Minus icon (shown when open)
                if MinusIcon then
                    AccordionIconMinus = New("ImageLabel", {
                        Image = MinusIcon.Url,
                        ImageColor3 = "AccentColor",
                        ImageRectOffset = MinusIcon.ImageRectOffset,
                        ImageRectSize = MinusIcon.ImageRectSize,
                        BackgroundTransparency = 1,
                        AnchorPoint = Vector2.new(0.5, 0.5),
                        Position = UDim2.new(1, -16, 0, 17),
                        Size = UDim2.fromOffset(16, 16),
            ...
            -- Smooth easing TweenInfo
            local SmoothTweenInfo = TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
            local FastTweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
            local IconTweenInfo = TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut)

            local function ResizeGroupbox()
                local contentHeight = GroupboxList.AbsoluteContentSize.Y + 14
                local targetHeight = Groupbox.IsOpen and (contentHeight + 36) or 36
                Background.Size = UDim2.new(1, 0, 0, targetHeight * Library.DPIScale)
            end
            ...
            function Groupbox:Toggle()
                if Groupbox.IsAnimating then return end
                Groupbox.IsAnimating = true
                Groupbox.IsOpen = not Groupbox.IsOpen
                
                local contentHeight = GroupboxList.AbsoluteContentSize.Y + 14
                local targetContainerHeight = Groupbox.IsOpen and contentHeight or 0
                local targetHolderHeight = Groupbox.IsOpen and (contentHeight + 36) or 36
                
                -- Animate icons with crossfade
                if AccordionIconPlus and AccordionIconMinus then
                    TweenService:Create(AccordionIconPlus, IconTweenInfo, {
                        ImageTransparency = Groupbox.IsOpen and 1 or 0,
                        Rotation = Groupbox.IsOpen and 90 or 0,
                    }):Play()
                    
                    TweenService:Create(AccordionIconMinus, IconTweenInfo, {
                        ImageTransparency = Groupbox.IsOpen and 0 or 1,
                        Rotation = Groupbox.IsOpen and 0 or -90,
                    }):Play()
                end

                -- Animate line visibility
                TweenService:Create(AccordionLine, FastTweenInfo, {
                    BackgroundTransparency = Groupbox.IsOpen and 0 or 1
                }):Play()

                -- Animate container height smoothly
                local containerTween = TweenService:Create(GroupboxContainer, SmoothTweenInfo, {
                    Size = UDim2.new(1, 0, 0, targetContainerHeight)
                })

                -- Animate holder height smoothly
                TweenService:Create(GroupboxHolder, SmoothTweenInfo, {
                    Size = UDim2.new(1, -4, 0, targetHolderHeight)
                }):Play()

                -- Animate background smoothly
                TweenService:Create(Background, SmoothTweenInfo, {
                    Size = UDim2.new(1, 0, 0, targetHolderHeight * Library.DPIScale + 4)
                }):Play()
                
                containerTween.Completed:Connect(function()
                    Groupbox.IsAnimating = false
                end)
            ...
            AccordionButton.MouseEnter:Connect(function()
                if AccordionIconPlus and not Groupbox.IsOpen then
                    TweenService:Create(AccordionIconPlus, FastTweenInfo, {
                        Size = UDim2.fromOffset(18, 18)
                    }):Play()
                end
                if AccordionIconMinus and Groupbox.IsOpen then
                    TweenService:Create(AccordionIconMinus, FastTweenInfo, {
                        Size = UDim2.fromOffset(18, 18)
                    }):Play()
                end
            end)
            ...
            AccordionButton.MouseLeave:Connect(function()
                if AccordionIconPlus then
                    TweenService:Create(AccordionIconPlus, FastTweenInfo, {
                        Size = UDim2.fromOffset(16, 16)
                    }):Play()
                end
                if AccordionIconMinus then
                    TweenService:Create(AccordionIconMinus, FastTweenInfo, {
                        Size = UDim2.fromOffset(16, 16)
                    }):Play()
                end
            ...
            AccordionButton.MouseButton1Click:Connect(function()
                Groupbox:Toggle()
            end)

            -- Handle content size changes smoothly
            GroupboxList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                if Groupbox.IsOpen and not Groupbox.IsAnimating then
                    local contentHeight = GroupboxList.AbsoluteContentSize.Y + 14
                    
                    TweenService:Create(GroupboxContainer, FastTweenInfo, {
                        Size = UDim2.new(1, 0, 0, contentHeight)
                    }):Play()
                    
                    TweenService:Create(GroupboxHolder, FastTweenInfo, {
                        Size = UDim2.new(1, -4, 0, contentHeight + 36)
                    }):Play()

                    TweenService:Create(Background, FastTweenInfo, {
                        Size = UDim2.new(1, 0, 0, (contentHeight + 36) * Library.DPIScale + 4)
                    }):Play()
                end
            end)
        ...
        function Tab:AddLeftAccordion(Name, IconName)
            return Tab:AddAccordion({ Side = 1, Name = Name, IconName = IconName })
        end

        function Tab:AddRightAccordion(Name, IconName)
            return Tab:AddAccordion({ Side = 2, Name = Name, IconName = IconName })
        end
